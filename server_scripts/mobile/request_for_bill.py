...
# - queue a PDF snapshot of that Draft Sales Invoice to the existing BCN Print Job
...
def render_sales_invoice_pdf(profile, sales_invoice):
    ...
    # BCN Print Job already stores a rendered PDF snapshot. Keep the existing
    # cashier print-format value only as queue metadata when no invoice-specific
    # print format is configured.
    metadata_format = (
...
def ensure_print_job(sales_order, sales_invoice, profile, current_user):
    ...
    existing_job_name = frappe.db.exists(
        "BCN Print Job",
    ...
    if existing_job_name:
        ...
        if (
            ...
            or existing_job.document_name != sales_order.name
        ):
    ...
    pdf, print_format = render_sales_invoice_pdf(profile, sales_invoice)
...
if sales_order.docstatus == 1 and restaurant_status == "Billing":
    ...
    for row in active_orders:
        if (
            ...
            row.docstatus == 1
            and row.custom_restaurant_status == "Billing"
        ):
